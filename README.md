...
| 14:00-21:00 | Apps Script 的 `runAutomation` 每 10 分钟按 `Asia/Shanghai` 读取当天 `web/report-status.json`。14:00 至 20:59 仅在 schema 3 机会闸门通过且图片哈希匹配时发送正常日报：active 模拟方案或平局预警任意一个即可，shadow-only 候选不算机会；两个有效计数均为零时没有投注方案且没有平局预警时不发送邮件，未知机会证据失败关闭并保持静默。正常发送路径只在 21:00 前重新验证当天日期。21:00 仅在已证明存在机会但日报仍未送达时发送当天唯一一封无附件失败通知。 |
